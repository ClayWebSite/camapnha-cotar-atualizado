...
export async function POST(request: Request) {
  try {
    ...
    const emailResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "Cotação Online <onboarding@resend.dev>",
        to: [operadoraInfo.email],
        reply_to: data.email,
        subject: `Nova Cotação de Plano ${operadoraInfo.nome} - ${data.nome}`,
        html: `
    ...
    if (!emailResponse.ok) {
      ...
      console.error("[v0] Erro ao enviar e-mail:", errorData)
    ...
    console.log("[v0] E-mail enviado com sucesso:", result.id)

    return NextResponse.json({ success: true, message: "Cotação enviada com sucesso!" })
  } catch (error) {
    console.error("[v0] Erro no processamento da cotação:", error)
